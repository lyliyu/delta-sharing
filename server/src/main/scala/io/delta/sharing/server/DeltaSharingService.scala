...
import io.delta.standalone.internal.{DeltaSharedTableLoader, RawData}
...
class DeltaSharingService(serverConfig: ServerConfig) {
  ...
  @Post("/shares/{share}/schemas/{schema}/tables/{table}/query_raw")
  @ConsumesJson
  def listRawFiles(
                 @Param("share") share: String,
                 @Param("schema") schema: String,
    ...
    val start = System.currentTimeMillis
    val tableConfig = sharedTableManager.getTable(share, schema, table)
    val urls = new RawData(tableConfig, serverConfig.preSignedUrlTimeoutSeconds).query()

    logger.info(s"Took ${System.currentTimeMillis - start} ms to load the table " +
      s"and sign ${urls.length} urls for table $share/$schema/$table")
    streamingUrls(1, urls)
  }
  ...
  private def streamingUrls(version: Long, urls: Seq[String]): HttpResponse = {
    val headers = createHeadersBuilderForTableVersion(version)
      .set(HttpHeaderNames.CONTENT_TYPE, DELTA_TABLE_METADATA_CONTENT_TYPE)
      .build()
    ResponseConversionUtil.streamingFrom(
