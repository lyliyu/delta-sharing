...
class RawDataReader:
    def __init__(self, table: Table, rest_client: DataSharingRestClient, names = None, dtype = None):
        self._table = table
        self._rest_client = rest_client
        self.names = names
        self.dtype = dtype

    def csv_to_pandas(self) -> pd.DataFrame:
        response = self._rest_client.list_files_in_rawdata(self._table)
        ...
        for file in response:
            ...
            pdf = pd.read_csv(file, header=None if self.names else "infer", names=self.names, dtype=self.dtype)
            pdfs.append(pdf)
    ...
    def json_to_pandas(self) -> pd.DataFrame:
        response = self._rest_client.list_files_in_rawdata(self._table)
        ...
        for file in response:
            file = loads(file)
            pdf = pd.read_json(file, dtype=self.dtype)
            pdfs.append(pdf)
...
def list_files_in_rawdata(self, table: Table):
    data: Dict = {}

    with self._post_internal(
            f"/shares/{table.share}/schemas/{table.schema}/tables/{table.name}/query_raw", data=data,
    ) as lines:
        return list(lines)
