...
from delta_sharing.reader import DeltaSharingReader, RawDataReader
...
def load_csv_as_pandas(url: str, cols: List[str] = None, datatype: typing.Dict = None) -> pd.DataFrame:
    """
    Load the shared CSV file using the give url as a pandas DataFrame.

    :param url: a url under the format "<profile>#<share>.<schema>.<table>"
    :param cols: a list of column names. If none, infer from data
    :param datatype: a dict of column types
    :return: A pandas DataFrame representing the shared CSV file.
    """
    profile_json, share, schema, table = _parse_url(url)
    profile = DeltaSharingProfile.read_from_file(profile_json)
...
def load_json_as_pandas(url: str, datatype: typing.Dict = None) -> pd.DataFrame:
    """
    Load the shared CSV file using the give url as a pandas DataFrame.
